      ...
      switch (entityRequest.getEntityType()) {
        ...
        case RequestType.CREATE_CASH_ORDER:
          if (isCashOrder(entityRequest)) {
            SrvCreateCashOrderRq request = CashOrderAdapter.requestCreateCashOrder(
        ...
        case RequestType.UPDATE_STATUS_CASH_ORDER:
          if (isCashOrder(entityRequest)) {
            SrvUpdStCashOrderRq request = CashOrderAdapter.requestUpdStCashOrder(
                (CashOrder) entityRequest.getRequestData());
            isEsbCache.putRequest(request.getHeaderInfo().getRqUID(), entityRequest);
            esbClient.sendMessage(asfsConverter.getXml(request));
          }
        ...
        case RequestType.CHECK_OVER_LIMIT:
          if (isCheckOverLimitRequest(entityRequest)) {
            SrvCheckOverLimitRq request = CheckOverLimitAdapter.requestCheckOverLimit(
        ...
        case RequestType.GET_WORKPLACE_INFO:
          if (isRequestDataString(entityRequest)) {
            SrvGetWorkPlaceInfoRq request = WorkPlaceAdapter.requestByWorkPlaceUId(
                (String) entityRequest.getRequestData());
            isEsbCache.putRequest(request.getHeaderInfo().getRqUID(), entityRequest);
            esbClient.sendMessage(asfsConverter.getXml(request));
          }
